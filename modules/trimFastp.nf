process trimFastp {
    ...
    script:
        ...
        def base_flags = "--length_required 20 --disable_quality_filtering --json ${sample_id}.json --html ${sample_id}.html"
        def window_flags = params.fastp_sliding_window ? "--cut_front --cut_tail --cut_window_size 4 --cut_mean_quality 20" : ""
    
        if (is_paired) """
          echo "Running fastp (paired-end) for ${sample_id} (sliding_window=${params.fastp_sliding_window})"
    
          fastp -i ${read_list[0]} -I ${read_list[1]} \
            -o ${sample_id}_R1.trim.fq.gz -O ${sample_id}_R2.trim.fq.gz \
            --detect_adapter_for_pe \
            ${base_flags} ${window_flags}
