// nextflow run main.nf -profile docker --bqsr true --variant_recalibration true --fastqc true --trimming "trim-galore" --trimming_only true
...
if (params.trimming == 'trim-galore') {
    include { trimTrimGalore } from './modules/trimTrimGalore'
} else if (params.trimming == 'trimmomatic') {
    include { trimTrimmomatic } from './modules/trimTrimmomatic'
} else if (params.trimming == 'fastp') {
    include { trimFastp } from './modules/trimFastp'
...
workflow {
    ...
    if (params.trimming == 'trim-galore') {
        ...
        trimTrimmomatic(reads_ch, adapter_pe_ch, adapter_se_ch)
    } else if (params.trimming == 'fastp') {
        trim_ch = trimFastp(read_pairs_ch)
    } else if (params.trimming == 'none') {
        trim_ch = read_pairs_ch
    } else {
        error "Unsupported trimming tool: ${params.trimming}. Please choose: trim-galore, trimmomatic, fastp or none."
    }
    ...
    if (params.runMode == 'align' | params.runMode == 'full') {

        // Align reads to the indexed genome
        if (params.aligner == 'bwa-mem') {
            align_ch = alignReadsBwaMem(trim_ch, indexed_genome_ch.collect())
        } else if (params.aligner == 'bwa-aln') {
            align_ch = alignReadsBwaAln(trim_ch, indexed_genome_ch.collect())
        } else if (params.aligner == 'bwa-mem2') {
            align_ch = alignReadsBwaMem2(trim_ch, indexed_genome_ch.collect())
        }
        ...
        stats_ch = samtoolsStats(indexed_bam_ch)
